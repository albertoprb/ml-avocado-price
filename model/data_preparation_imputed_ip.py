...
output_path = os.path.join(os.path.dirname(__file__), "../data/slices_imputer/")

df_avocado = pd.read_csv(
    os.path.normpath(input_path + "avocado.csv")
)
df_imputed = pd.read_csv(
    os.path.normpath(input_path + "imputer/imputed_2018_dec.csv")    
)
df = pd.concat([df_imputed,df_avocado])
df.to_csv(output_path + 'avocado_imputed.csv', index=False)
